...
%     tform(3,4) = max(abs(coord_frontal(3,:)))*10;
    ...
    if contains(render,'F')
        if verbose
            ...
            if ~isempty(lmksF)
                [~, ~, img, map, lmksF_img] = z_buffering_modif(myMesh.verts, myMesh.faces, myTexture, cam, tform, dist, 'scale_for_imgSize', scale_for_imgSize, 'Landmarks',lmksF );
            else
    ...
    if contains(render,'L')
        ...
        if verbose
            ...
            if ~isempty(lmksL)
                [~, ~, img, map, lmksR_img] = z_buffering_modif(coord_right, myMesh.faces, myTexture, cam, tform, dist, 'scale_for_imgSize', scale_for_imgSize, 'Landmarks',lmksL );
            else
                [~, ~, img, map] = z_buffering_modif(coord_right, myMesh.faces, myTexture, cam, tform, dist, 'scale_for_imgSize', scale_for_imgSize);
                lmksR_img = [];
            end
        end
    ...
    if contains(render,'R')
        ...
        if verbose
            ...
            if ~isempty(lmksR)
                [~, ~, img, map, lmksL_img] = z_buffering_modif(coord_left, myMesh.faces, myTexture, cam, tform, dist, 'scale_for_imgSize', scale_for_imgSize, 'Landmarks',lmksR );
            else
                [~, ~, img, map] = z_buffering_modif(coord_left, myMesh.faces, myTexture, cam, tform, dist, 'scale_for_imgSize', scale_for_imgSize);
                lmksL_img = [];
            end
        end
        ...
        if isempty(outFile)
            figure; imshow(uint8(img));
            if ~isempty(lmksL_img), hold on, plot2pts(lmksL_img', '*r'); end % If you stop here, you'll viualize the landmarks
        else
