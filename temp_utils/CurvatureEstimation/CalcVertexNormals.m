...
e0=FV.verts(FV.faces(:,3),:)-FV.verts(FV.faces(:,2),:);
e1=FV.verts(FV.faces(:,1),:)-FV.verts(FV.faces(:,3),:);
e2=FV.verts(FV.faces(:,2),:)-FV.verts(FV.faces(:,1),:);
...
Avertex=zeros(size(FV.verts,1),1);

% Calculate Vertice Normals
VertexNormals=zeros([size(FV.verts,1) 3]);
up=zeros([size(FV.verts,1) 3]);
vp=zeros([size(FV.verts,1) 3]);
